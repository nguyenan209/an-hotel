  ...
  return (
    <Card className="overflow-hidden h-full flex flex-col">
      ...
      <CardContent className="flex-1 p-4">
        ...
        <h3 className="mt-2 text-lg font-semibold leading-tight line-clamp-1">{homestay.name}</h3>
        <div className="mt-2 flex items-center text-sm text-muted-foreground">
          <Users className="mr-1 h-4 w-4" />
          <span>Tối đa {homestay.maxGuests} khách</span>
        </div>
        <p className="mt-2 text-lg font-bold">
          {formatCurrency(homestay.price)} <span className="text-sm font-normal text-muted-foreground">/ đêm</span>
        </p>
      </CardContent>

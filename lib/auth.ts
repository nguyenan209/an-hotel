...
export const comparePassword = async (password: string, hashedPassword: string): Promise<boolean> => compare(password, hashedPassword);
export const generateToken = (userId: string, role: string): string => sign({ userId, role }, JWT_SECRET, { expiresIn: JWT_EXPIRES_IN });

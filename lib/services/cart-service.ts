...
export const cartAPI = {
  ...
  async addToCart(customerId: string, item: Partial<CartItem>) {
    const response = await fetch(`/api/cart/${customerId}`, {
      method: "POST",
  ...
  async updateItemNote(customerId: string, homestayId: string, note: string) {
    const response = await fetch(`/api/cart/${customerId}/note`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
  ...
  async clearCart(customerId: string) {
    const response = await fetch(`/api/cart/${customerId}/clear`, {
      method: "DELETE",
    });
